...
<input type="hidden" name="roomGrade" value="${dto.roomGrade}">
	    ...
	    <c:forEach items="${list}" var="vo">
		    ...
		    <c:if test="${sw}">
				<button type="button" value="${vo.roomNum}" class="roomBtn" 
				    style="color:${vo.roomNum==dto.roomNum?'white':'black'}; margin-top:5px; background:${vo.roomNum==dto.roomNum?'#08298A':'white'}; font-size: 12px;">
				    ${vo.roomGrade}<br>${vo.roomNum}호차
				</button>
		    </c:if>
	    </c:forEach>
    ...
    <div style="text-align:right; margin: 10px;">
    	<i style=" color:#08298A; font-size: 25px;" class="fas fa-user seatClick"></i>
    ...
    <div style="width: 100%;">
		...
		<div style="padding-right:20px; width: 50%;text-align: right; float: left;">
			<button type="button" onclick="confirmBtn();" style="font-weight:bold; font-size:15px; border-radius:5px; width:150px; height:30px; background: #08298A; color: white; border: none;">선택좌석 예약하기</button>
		</div>
	</div>
</form>
</div>
